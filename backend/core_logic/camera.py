...
def log_stress(level, prob):
    try:
        db_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'database.db')
        conn = sqlite3.connect(db_path)
        cur = conn.cursor()
        # Logging stress for Raja (ID 1) by default in video feed
        cur.execute("INSERT INTO stress_logs (elephant_id, stress_level, stress_probability) VALUES (1, ?, ?)", (int(level*100), prob))
        conn.commit()
    except Exception as e:
        print(f"Error logging stress: {e}")
    finally:
        if 'conn' in locals():
            conn.close()
